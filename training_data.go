...
SELECT id_nasabah, nama_lengkap FROM nasabah WHERE UPPER(nama_lengkap) LIKE UPPER('a%');
`,
	`
...
GROUP BY n.id_nasabah, n.nama_lengkap ORDER BY total_saldo DESC
...
FROM jurnal_transaksi jt JOIN transaksi t ON jt.id_transaksi = t.id_transaksi
...
GROUP BY n.id_nasabah, n.nama_lengkap ORDER BY total_saldo_tabungan DESC
FETCH FIRST 1 ROWS ONLY;
`,
	`
...
FROM transaksi t
WHERE t.waktu_transaksi >= TRUNC(ADD_MONTHS(SYSDATE, -1), 'MM')
  AND t.waktu_transaksi < TRUNC(SYSDATE, 'MM')
